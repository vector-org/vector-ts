...
export const Publishing = {
  ...
  fromJSON(object: any): Publishing {
    return {
      ...
      protoReferenceDocumentationUri: isSet(object.protoReferenceDocumentationUri) ? String(object.protoReferenceDocumentationUri) : ""
    };
  },
...
export const DotnetSettings = {
  ...
  fromJSON(object: any): DotnetSettings {
    return {
      common: isSet(object.common) ? CommonLanguageSettings.fromJSON(object.common) : undefined,
      renamedServices: isObject(object.renamedServices) ? Object.entries(object.renamedServices).reduce<{
        [key: string]: string;
      }>((acc, [key, value]) => {
        acc[key] = String(value);
        return acc;
      }, {}) : {},
      renamedResources: isObject(object.renamedResources) ? Object.entries(object.renamedResources).reduce<{
        [key: string]: string;
      }>((acc, [key, value]) => {
        acc[key] = String(value);
        return acc;
      }, {}) : {},
      ignoredResources: Array.isArray(object?.ignoredResources) ? object.ignoredResources.map((e: any) => String(e)) : [],
      forcedNamespaceAliases: Array.isArray(object?.forcedNamespaceAliases) ? object.forcedNamespaceAliases.map((e: any) => String(e)) : [],
      handwrittenSignatures: Array.isArray(object?.handwrittenSignatures) ? object.handwrittenSignatures.map((e: any) => String(e)) : []
    };
  },
...
function createBaseMethodSettings(): MethodSettings {
  return {
    selector: "",
    longRunning: LongRunning.fromPartial({})
  };
}
