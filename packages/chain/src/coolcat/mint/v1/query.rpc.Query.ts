...
export class QueryClientImpl implements Query {
  ...
  params = async (request: QueryParamsRequest = {}): Promise<QueryParamsResponse> => {
    ...
    const promise = this.rpc.request("coolcat.mint.v1.Query", "Params", data);
    return promise.then(data => QueryParamsResponse.decode(new BinaryReader(data)));
  };
  /* Inflation returns the current minting inflation value. */
  inflation = async (request: QueryInflationRequest = {}): Promise<QueryInflationResponse> => {
    const data = QueryInflationRequest.encode(request).finish();
    const promise = this.rpc.request("coolcat.mint.v1.Query", "Inflation", data);
  ...
  annualProvisions = async (request: QueryAnnualProvisionsRequest = {}): Promise<QueryAnnualProvisionsResponse> => {
    ...
    const promise = this.rpc.request("coolcat.mint.v1.Query", "AnnualProvisions", data);
